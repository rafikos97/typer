...
import { fetchUserInformation, fetchUserInformationSuccess } from "./user-panel.actions";
...
export class UserPanelEffects {
    ...
    readonly fetch$ = createEffect(() => this.actions$.pipe(
        ofType(fetchUserInformation),
        switchMap(() => this.userPanelService.fetchUserInformation()),
        map((userInformation: UserInformation) => fetchUserInformationSuccess({ userInformation }))
    )
}
